...
public class PwdFindAction extends AbstractController {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if ("POST".equalsIgnoreCase(method)) {
			...
			boolean sendMailSuccess = true; //메일이 정상적으로 전송되었는지 확인하기 위한 용도의 변수
			...
			if (isUserExist) {
				...
				// 예: certificationCod ==> dngrn475003
				...
				for (int i = 0; i < 5; i++) {
					...
					randchar = (char) (rnd.nextInt('z' - 'a' + 1) + 'a'); // 메소드 연산하면서 char->int타입으로 바뀜
					certificationCode += randchar;
				} // end of for ----------------------------------------------

				int randnum = 0;
				for (int i = 0; i < 7; i++) {
					randnum = rnd.nextInt(9 - 0 + 1) + 0; // 0부터9 까지 랜덤하게 뽑아오기
				...
				// 랜덤하게 생성한 인증코드(certificationCode)를 비밀번호 찾기를 하고자하는 사용자의 email로 전송시킨다.
				GoogleMail mail = new GoogleMail();

				try {
					mail.sendmail(email, certificationCode); // mail 전송
					
					// mail이 전송되고 난 후 세션불러오기
