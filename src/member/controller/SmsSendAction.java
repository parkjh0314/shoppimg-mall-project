...
public class SmsSendAction extends AbstractController {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		  //String api_key = "발급받은 본인의 API Key";  // 발급받은 본인 API Key
			String api_key = "NCS3H9BI7TBB5FVW";  // 박지현꺼임
		  
		  //String api_secret = "발급받은 본인의 API Secret";  // 발급받은 본인 API Secret
			String api_secret = "EIW49UZL0HN2Z4GLZYWXCLFG7HNRWJ01";  // 박지현꺼임
			...
			// == 4개 파라미터(to, from, type, text)는 필수사항이다. == // 아래의 키값은 API에서 넣어준거라 건들면 안됨
			HashMap<String, String> paraMap = new HashMap<String, String>();
			paraMap.put("to", mobile); // 수신번호
			paraMap.put("from", "01033704737"); // 발신번호
			...
			// org.json.JSONObject이 아니라
			// org.json.simple.JSONObject 이어야 한다.
			// 다른건 위에거 쓰는게 좋지만 문자는 꼭 아래걸로 사용
			
			String json = jsobj.toString();
			
			//System.out.println("~~~ 확인용 json => " + json);
			//{"group_id":"R2Gx8ut4auVrqQsv","success_count":1,"error_count":0}
