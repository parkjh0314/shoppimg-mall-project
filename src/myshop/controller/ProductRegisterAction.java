...
public class ProductRegisterAction extends AbstractController {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// == 관리자(admin)으로 로그인 했을때만 제품등록이 가능하도록 한다. == //
		...
		if(loginuser != null && "admin".equals(loginuser.getUserid())) {
			...
			if(!"POST".equalsIgnoreCase(method)) { //GET 이라면
				...
				/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			     ...
			     HttpServletRequest request 을 사용해서는 데이터값(첨부파일)을 받아올 수 없다.
				...
				*/
					...
					// === 첨부되어지는 이미지 파일이 올라가는 절대경로 imagesDir ==> C:\NCS\workspace(jsp)\.metadata\.plugins\org.eclipse.wst.server.core\tmp0(또는tmp1)\wtpwebapps\MyMVC\images
				/*
				    MultipartRequest의 객체가 생성됨과 동시에 파일 업로드가 이루어 진다.
							 
				    MultipartRequest(HttpServletRequest request,
					       	         String saveDirectory, -- 파일이 저장될 경로(실제경로)
				...
				// === 첨부 이미지 파일을 올렸으니 그 다음으로 제품정보를 (제품명, 정가, 제품수량,...) DB의 tbl_product 테이블에 insert 를 해주어야 한다.  ===
				  ...
				  String pimage1 = mtrequest.getFilesystemName("pimage1"); //첨부된 filename을 얻어옴
				...
				*/
				  ...
				  pcontent = pcontent.replaceAll("<", "&lt;"); // 태그의 부등호를 그냥 하나의 문자열로 변경하여 태그자체가 작동하지 않게 하기=> DB에 &lt;로 들어감
				  ...
				  //String[] arr_attach = mtrequest.getParameterValues("attach"); // name이 attach인게 여러개: 배열로 받아옴
				  ...
				  for(int i=0; i<attachCount; i++) {
					  ...
					  m = pdao.product_imagefile_Insert(pnum, attachFileName);// pnum은 위에서 채번해온 제품번호
