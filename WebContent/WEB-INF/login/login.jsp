...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("input#loginPwd").keyup(function(event){ //암호입력란에서 엔터를 했을 때
			if(event.keyCode==13) { //keyup이벤트가 일어났을때 keyCode가 13(엔터)일때
				goLogin(); // 로그인 시도
			}
		});
    ...
    */
	...
	function goCoinPurchaseTypeChoice(userid){
		...
		window.open(url, "coinPurchaseTypeChoice", "left=450px, top=100px, width=650px, height=570px");
	...
	function goCoinPurchaseEnd(userid, coinmoney) {
		...
		window.open(url, "coinPurchaseTypeEnd", "left=450px, top=100px, width=820px, height=600px");
	}
	
	// === DB상의 tbl_member 테이블에 해당 사용자의 코인금액을 증가(update)시켜주는 함수 === // 
	function goCoinUpdate(userid, coinmoney) {
		
		alert("확인용 사용자ID : "+userid+" 결제금액 : "+coinmoney+"원");
	...
	function goEditPersonal(userid) {
		// 나의 정보수정하기 팝업창 띄우기
	      ...
	      window.open(url, "memberEdit", "left=350px, top=100px, width=800px, height=650px");
...
<c:if test="${not empty sessionScope.loginuser}">
 ...
 <%-- ****** 아이디 찾기 Modal ****** --%>
  ...
  <%-- PG(Payment Gateway 결제)에 코인금액을 카드(카카오페이등)으로 결제 후 DB상에 사용자의 코인액을 업데이트 해주기위한 폼 --%>
