...
<script type="text/javascript">
   $(document).ready(function() {
	  ...
	  goCommentListView(); //제품 구매후기를 보여주는 함수 호출
	  goLikeDislikeCount(); // 좋아요, 싫어요 갯수를 보여주도록 하는 함수호출. 
	...
	<%--	   // 보내야할 데이터를 선정하는 첫번째 방법
		   var queryString = {"fk_userid" : '${sessionScope.loginuser.userid}',    //세션에서 가져와서 넣어주면서 ''안에 넣어 문자열로 만들어주기
				   			  "fk_pnum" : ${pvo.pnum}							   //getAttribute로 받아온 거
		   					  "contents" : $("textarea#commentContents").val() };  //태그의 value값
	--%>
		   
		   // 보내야할 데이터를 선정하는 두번째 방법
		   var queryString = $("form[name=commentFrm]").serialize(); // jQuery에서 사용하는 것으로써, form태그의 선택자.serialize(); 을 해주면 form 태그내의 모든 값들을 name값을 키값으로 만들어서 보내준다. 
   ...
   function goCommentListView() {
	   ...
	   $.ajax({
		   ...
		   type:"GET",
		   data:{"fk_pnum":${pvo.pnum}}, //특정 제품에 대한 후기를 보여줘야 하므로 테이블에서 제품번호로 찾기위해 보내줌
		   dataType:"JSON",
		   success:function(json) {
			 //{"contents":"제품후기내용", "name":"작성자이름", "writeDate":"작성일자"} {"contents":"제품후기내용2", "name":"작성자이름2", "writeDate":"작성일자2"}
			    var html = "";
	   ...
	   $.ajax({
		...
		// type:"GET",
		   ...
		   success:function(json) {
			   ...
			   $("div#dislikeCnt").html(json.dislikecnt);ss
		   },
		   error: function(request, status, error){
				alert("code: "+request.status+"\n"+"message: "+request.responseText+"\n"+"error: "+error);
		   }
